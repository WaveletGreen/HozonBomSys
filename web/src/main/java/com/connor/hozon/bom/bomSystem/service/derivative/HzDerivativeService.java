...
public class HzDerivativeService {
    /**
     * 工厂
     */
    @Autowired
     ...
     * 日志
     */
    private static Logger logger = LoggerFactory.getLogger(HzDerivativeService.class);
    ...
    public void saveCompose(HzDerivativeRequestDTO hzDerivativeRequestDTO, JSONObject results) {
...
//        List<HzFullCfgModel> modelCfgs = fullCfgModelDao.selectByModelUid(hzDerivativeRequestDTO.getModelUid());
        if (basic1 != null) {
        ...
        HzFactory factory = hzFactoryDAO.findFactory(null, hzDerivativeRequestDTO.getFactoryCode());
        if (factory == null) {
        ...
        HzCfg0ModelRecord cmodel = hzCfg0ModelServiceImpl.getModelByPuid(hzDerivativeRequestDTO.getModelUid());
        ...
        List<String> column = hzCfg0OptionFamilyService.doGetColumnDef(hzDerivativeRequestDTO.getProjectUid(), "<br/>");
        List<HzMaterielFeatureBean> hzMaterielFeatureBeans = hzFeatureServiceImpl.doSelectMaterielFeatureByProjectPuid(hzDerivativeRequestDTO.getProjectUid());
        ...
        if (hzMaterielFeatureBeans == null || column == null || column.size() == 0) {
            ...
            hzMaterielFeatureBeans.stream()
                    .filter(bean -> bean.getpCfg0ModelRecord() != null && bean.getpCfg0ModelRecord().equals(hzDerivativeRequestDTO.getModelUid()))
    ...
    public void saveCompose2(HzDerivativeRequestDTO hzDerivativeRequestDTO, JSONObject results) {
        HzDerivativeMaterielBasic basic1 = hzDerivativeMaterielBasicDao.selectByModelAndColorUid(hzDerivativeRequestDTO.getModelUid(), hzDerivativeRequestDTO.getColorModel());
        if (basic1 != null) {
            results.put("msg", "已存在相同配置的衍生物料，请进行修改操作或先删除衍生物料");
            results.put("status", false);
        ...
        HzFeatureValue shell = hzFeatureServiceImpl.doSelectByCodeAndDescWithMainItem(local);

        if (shell == null) {
            /**强制设置为中文描述“车身颜色”*/
            local.setpCfg0FamilyDesc(SpecialFeatureOptions.CSNAME.getDesc());
            if ((shell = hzFeatureServiceImpl.doSelectByCodeAndCnDescWithMainItem(local)) == null) {
         ...
         * 如果超级物料为空，则存储超级物料
         */
        HzMaterielRecord sm = hzSuperMaterielServiceImpl.doSelectByProjectPuid(hzDerivativeRequestDTO.getProjectUid());
        if (null == sm) {
        ...
        for (HzFullCfgModel modelCfg : modelCfgs) {
            if ((record = hzFeatureServiceImpl.doSelectOneByPuid(modelCfg.getModCfg0Uid())) != null) {
     ...
     *
     * @param hzDerivativeRequestDTO ProcessReceiveDto
     * @param feature        pojo
     * @return pojo
     */
    public static HzCfg0ModelFeature ModelFeatureDtoToPojo(HzDerivativeRequestDTO hzDerivativeRequestDTO, HzCfg0ModelFeature feature) {
...
//        List<HzFeature> columns = hzCfg0OptionFamilyService.doGetCfg0OptionFamilyListByProjectPuid(projectUid);
        ...
        List<HzFeatureValue> innerColor = hzFeatureServiceImpl.doSelectByDescAndProjectId(param);
