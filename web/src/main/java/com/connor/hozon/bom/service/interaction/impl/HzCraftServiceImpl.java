...
package com.connor.hozon.bom.service.interaction.impl;
...
import com.connor.hozon.bom.service.interaction.HzCraftService;
        ...
        while (iterator.hasNext()) {
            ...
            if (parent != null && checkString(parent.getLineId())) {
                ...
                if (ohMyChildren == null || ohMyChildren.isEmpty()) {
                    log.warn("Oops," + parent.getLineId() + "(PUID:" + parent.getPuid() + ")下的子删除或剪切了");
                    continue;
                }
                //找到所有的父行
                ...
                if (allCodeIsWithToParentCode != null && !allCodeIsWithToParentCode.isEmpty()) {
                    for (int i1 = 0; i1 < allCodeIsWithToParentCode.size(); i1++) {
                        ...
                        if (isWithsChildren.size() != ohMyChildren.size()) {
                            log.warn("PBOM BOM行的PUID为" + allCodeIsWithToParentCode.get(i1).getPuid() +
...
//
//                        } else {
//                            log.warn("虽然PBOM中PUID为" + allCodeIsWithToParentCode.get(i1).getId() +
            ...
            for (String _key : toChildrenNeedToUpdateItsLineIndex.get(key).keySet()) {
                if (craftIsInfluenceToDB) {
                    ...
                    if (hzPbomRecordDAO.update(toUpdate) <= 0) {
                        log.error("更新需要重新排序的子层" + record.getLineId() + "查找编号失败");
                    }
                }
            }
        }
...
//            listOfDto.add(dto);
        ...
        if (craftIsInfluenceToDB) {// 已生效的进行状态标记 未生效的直接删除
            ...
            if (!WriteResultRespDTO.isSuccess(respDTO)) {
                log.error(respDTO.getErrMsg() + "->删除合成源数据失败");
            }
...
//                                            pLineIndex += 10;
                            ...
                            //直接零件层
                                ...
                                //永远没有找到父层，那一定是第一层
                                    ...
                                    log.error("没有子层的合成源找不到父层UID，但它本身的父层一定是存在的");
                                }
                            }
                            //克隆一份出来
    ...
    private void doReset() {
        for (Map.Entry<String, Map<String, HzPbomLineRecord>> entry : toChildrenNeedToUpdateItsLineIndex.entrySet()) {
            ...
            if (theKeyToAllSameParent.containsKey(key)) {
                ...
                log.warn("没有找到有相同父的映射key，则没有找到排序好后的开始影响的pBom的查找编号");
                continue;
            }
            ///
            if (value != null && !value.isEmpty()) {
                ...
                log.warn("需要重置查找编号的数据量map为空，key=" + key);
            }
        }
        log.warn("重置受影响的子BOM查找编号成功");
    }
