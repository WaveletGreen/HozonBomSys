...
import com.connor.hozon.service.bom.resourcesLibrary.dictionaryLibrary.HzDictionaryLibraryService;
...
@Slf4j
public class HzFeatureController {
    /***特性值服务*/
    @Autowired
    private FeatureValueService featureValueService;
    /***主配置Dao层*/
    @Autowired
    private HzMainConfigDao hzMainConfigDao;
    /*** 同步特性，不再在当前controller中使用*/
    @Autowired
    private SynFeatureService synFeatureService;
    /***项目下的特性，值针对当前项目的特性，更高一层的是字典库*/
    @Autowired
    private FeatureServiceImpl cfg0OptionFamilyService;
    ...
    public Map<String, Object> loadCfg0(@RequestParam("projectPuid") String projectPuid, HzFeatureQuery queryBase) {
        return featureValueService.loadFeature(projectPuid, queryBase);
    }
    ...
    public JSONObject add2(@RequestBody HzFeatureValue record) {
        ...
        if (!featureValueService.preCheck(record)) {
            result.put("status", false);
        ...
        HzFeature _family = cfg0OptionFamilyService.selectByCodeAndDescWithMain(family);
        //检查当前项目有没有用到特定的特性
        if (_family == null) {
            family.setId(UUIDHelper.generateUpperUid());
            //没有用到，则进行特性插入
            if (!cfg0OptionFamilyService.insert(family)) {
                result.put("status", false);
        ...
        //将特性值插入到表中
        if (featureValueService.insertOne(record)) {
...
//            }
            record = featureValueService.selectOneByPuid(record.getPuid());
    ...
    public JSONObject deleteByPuidFake(@RequestBody List<HzFeatureValue> records) {
        ...
        for (HzFeatureValue record : records) {
            ...
            //原始配置先不给删除，只能删除新加的配置项
                ...
                if (featureValueService.selectOneByPuid(record.getPuid()) != null) {
                    //如果需要删除原数据
                    toDelete.add(record);
    ...
    public JSONObject deleteByPuid(@RequestBody List<HzFeatureValue> records) throws Exception {
        ...
        for (HzFeatureValue record : records) {
            ...
            //原始配置先不给删除，只能删除新加的配置项
                ...
                /*if (featureValueService.doSelectOneAddedCfgByPuid(record.getId()) != null) {
                    toDelete.add(record);
                }
                else*/
                if (featureValueService.selectOneByPuid(record.getPuid()) != null) {
...
//        HzFeature family = new HzFeature();
        ...
        for (int i = 0; i < _toDelete.size(); i++) {
            ...
            List<HzFeatureValue> children = featureValueService.selectByFamilyUidWithProject(_toDelete.get(i).getpCfg0FamilyPuid(), main.getProjectId());
            if (children.size() <= 1 && children.get(0).getPuid().equals(_toDelete.get(i).getPuid())) {
//                family.setId(_toDelete.get(i).getpCfg0FamilyPuid());
                log.warn("正在自动删除特性");
                if (cfg0OptionFamilyService.deleteByPrimaryKey(_toDelete.get(i).getpCfg0FamilyPuid())) {
        ...
        if (_toDelete.size() > 0) {
            log.warn(featureValueService.deleteCfgByList(_toDelete) ? "删除特性多个" : "删除失败或已经删除族");
        }
        result.put("status", true);
        result.put("msg", "删除成功");
    ...
    /**
     * 获取添加特性页面，已废除，采用{@link HzFeatureController#addPage2(String, Model)}代替
     *
     * @param projectPuid 项目UID
     * @return 添加特性页面
     */
    ...
    public String modifyPage(@RequestParam("projectPuid") String puid, Model model) {
        HzFeatureValue record = featureValueService.selectOneByPuid(puid);
        if (record == null) {
//            record = featureValueService.doSelectOneAddedCfgByPuid(puid);
//            if (record == null) {
...
//            }
    ...
    /**
     * 已废除
     * 该方法可以直接添加特性到数据库中，缺少配置字典的约束，请采用{@link HzFeatureController#add2(HzFeatureValue)}代替
     *
     * @param record
     * @return
     * @throws Exception
...
//            }
    ...
    public JSONObject modify(@RequestBody HzFeatureValue record) {
        ...
        if (featureValueService.selectOneByPuid(record.getPuid()) != null) {
            if (featureValueService.update(record)) {
                //重新取回一遍数据
                record = featureValueService.selectOneByPuid(record.getPuid());
...
//        else if (featureValueService.doSelectOneAddedCfgByPuid(record.getId()) != null) {
//            if (featureValueService.doUpdateAddedCfg(record)) {
//                result.put("msg", "更新特性值" + record.getFeatureValueCode() + "成功");
//            } else {
...
//        }
    ...
    public Map<String, Object> loadRelevance(@RequestParam("projectPuid") String projectPuid) {
        ...
        int _index = 0;
        _index = featureValueService.doLoadRelevance(projectPuid, _list, _index, "HZ_CFG0_RECORD");
//        featureValueService.doLoadRelevance(projectPuid, _list, _index, "HZ_CFG0_ADD_CFG_RECORD");
...
//        featureValueService.doLoadRelevance(projectPuid, _list, _index, "HZ_CFG0_ADD_CFG_RECORD");
    ...
    public boolean relOption(@RequestBody HzRelevanceResponseDTO bean) {
        ...
        record = featureValueService.selectOneByPuid(bean.getPuid());
        if (record != null) {
            record = featureValueService.selectOneByPuid(bean.getPuid());
            record.setpCfg0Relevance(bean.getRelevanceCode());
            return featureValueService.update(record);
        }
//        else if ((record = featureValueService.doSelectOneAddedCfgByPuid(bean.getId())) != null) {
//            record.setpCfg0Relevance(bean.getRelevanceCode());
//            return featureValueService.doUpdateAddedCfg(record);
//        }
        else {
            return false;
        }
    ...
    public String relPage(@RequestParam("uid") String uid, @RequestParam("page") String page, Model model) {
        ...
        if ("addPage".equals(page)) {
            ...
            HzFeatureValue record = featureValueService.selectOneByPuid(uid);
            if (record == null) {
//                record = featureValueService.doSelectOneAddedCfgByPuid(uid);
//                if (record == null) {
                model.addAttribute("msg", "没有找到对应的特性数据，请重试或联系系统管理员!");
                return "errorWithEntity";
//                }
