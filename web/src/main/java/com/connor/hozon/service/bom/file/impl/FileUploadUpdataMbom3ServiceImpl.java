...
package com.connor.hozon.service.bom.file.impl;
...
import com.connor.hozon.service.bom.file.FileUploadUpdataMbom3Service;
...
@Service("fileUploadUpdataMbom3Service")
public class FileUploadUpdataMbom3ServiceImpl implements FileUploadUpdataMbom3Service {
    @Autowired
    ...
    public WriteResultRespDTO UploadMbomToDB(MultipartFile file, String projectId) {
        ...
        try {
            ...
            for(int numSheet = 0; numSheet < workbook.getNumberOfSheets(); numSheet++ ){
                ...
                if(map.size()>0){
                    //返回零件号
                    StringBuffer stringBuffer = new StringBuffer("白车身财务BOM导入文件解析失败!</br>");
                    ...
                    //while (i<=sheet.getLastRowNum()){
                        for(int rowNum = 1; rowNum <= sheet.getLastRowNum(); rowNum++){
                            ...
                            if(sheetRow != null){
                                ...
                                //updateInput
                                hzMbomRecordDAO.updateInputFinance(hzMbomLineRecords.get(rowNum-1));
                                //i++;
                            }
                        }
                    //}
