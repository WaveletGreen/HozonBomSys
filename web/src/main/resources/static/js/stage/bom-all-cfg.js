...
function fixTable(myData) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        var delta = "<tr id='tr" + j + "' >" +
            ...
            "</td>"
            // "<td style='text-align: center'><input class='btn btn-default' type='button' value='编辑' style='width: 50px' onclick='editorOrSave(this)'><input class='featureValueCode' type='text' value='"+dataOfModel.featureValueCode+"' hidden='hidden' disabled='disabled'><div id='"+cfgId+"' style='display: none'>"+dataOfModel.bomLinePuid+"</div></td>"
            +
            "<td>" +
        ...
        for (var index in dataOfModel) {
            ...
            if (aaa == 2) {
                ...
                for (var j = 0; j < array.length; j++) {
                    var cfg = array[j];
                    if (cfg.featureValueCode == dataOfModel[index]) {
                        cfgSelect += "<option value='" + cfg.puid + "|" + cfg.pCfg0Desc + "' selected='selected' style='color: #c1ab58'>" + cfg.featureValueCode + "</span></option>";
                    } else {
                        cfgSelect += "<option value='" + cfg.puid + "|" + cfg.pCfg0Desc + "' style='color: #3fc3ee'>" + cfg.featureValueCode + "</span></option>";
                    }
                }
                cfgSelect += "</select>";
                // delta = delta + "<td class='edit'><input type='text' value='"+dataOfModel[index]+"' style='display: none'><div style='width: 150px'>" + dataOfModel[index] + "</div></td>";
...
/**
 *表格渲染,渲染input的palceholder和快速查找
 */
function tableInputRendering() {
                        ...
                        success: function (result) {
                            ...
                            $(but).parent().siblings().each(function (index, item) {
                                if (index == 8) {
                                    ...
                                    for (let i = 0; i < _array.length; i++) {
                                        $(select).append("<option value='" + _array[i].puid + "|" + _array[i].pCfg0Desc + "'><span style='color: #0c5460'>" + _array[i].featureValueCode + "</span></option>");
                                    }
                                    if (undefined != selfCfg) {
                                        for (let cfgIdIndex = 0; cfgIdIndex < array.length; cfgIdIndex++) {
                                            if (array[cfgIdIndex].puid == selfCfg.cfgCfg0Uid) {
                                                $(select).append("<option value='" + array[cfgIdIndex].puid + "|" + array[cfgIdIndex].pCfg0Desc + "' selected='selected'><span style='color: #b96104'>" + array[cfgIdIndex].featureValueCode + "</span></option>");
                                            }
                                        }
                                    }
