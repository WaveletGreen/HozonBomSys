...
var toolbar = [
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (!result.success) {
                        ...
                        window.Ewin.dialog({
                            title: "添加",
                            url: "feature/addPage2?projectPuid=" + projectPuid,
                            gridId: "gridId",
                            width: 400,
                            height: 500
                        })
        ...
        handler: function () {
            ...
            for (let i in rows) {
                if (1 == rows[i].cfgIsInProcess || "1" == rows[i].cfgIsInProcess) {
                    window.Ewin.alert({message: rows[i].featureValueCode + "已在流程中，不允许删除"});
                    return false;
                }
            }

        var url = "feature/deleteByPuidFake";
        $.ajax({
            url: "privilege/write?url=" + url,
            type: "GET",
            success: function (result) {
    ...
    },
    {
        ...
        handler: function () {
            ...
            for (let i in rows) {
                if (1 == rows[i].cfgIsInProcess || "1" == rows[i].cfgIsInProcess) {
                    window.Ewin.alert({message: rows[i].featureValueCode + "已关联变更单"});
                    return false;
                }else if(1 == rows[i].cfgStatus || "1" == rows[i].cfgStatus){
                    window.Ewin.alert({message: rows[i].featureValueCode + "已生效，不能关联变更单"});
                    return false;
                }
                msg += "<p>" + rows[i].featureValueCode + "-" + rows[i].pCfg0Desc + "</p>";
            }
            msg += "</div>";
    ...
    },
    {
        ...
        handler: function () {
            ...
            for (let i in rows) {
                if (1 == rows[i].cfgIsInProcess || "1" == rows[i].cfgIsInProcess) {
                    window.Ewin.alert({message: rows[i].featureValueCode + "已关联变更单，不允许撤销"});
                    return false;
                }
                if(1 == rows[i].cfgStatus || "1" == rows[i].cfgStatus ){
                    window.Ewin.alert({message: rows[i].featureValueCode + "已生效，不允许撤销"});
                    return false;
                }
            }
...
function loadData(_projectPuid) {
    ...
    $table.bootstrapTable({
        url: "feature/loadFeature?projectPuid=" + projectPuid,
        method: "GET",
